...
import type {
  ...
  BitcoreBlockHeader,
  BitcoreTransaction,
} from 'bitcore-p2p-bsv';
import bitcoreP2pSV, { BitcoreInventoryType } from 'bitcore-p2p-bsv';
...
const { internalBitcore } = bitcoreP2pSV;
// eslint-disable-next-line @typescript-eslint/naming-convention
const { Peer } = bitcoreP2pSV;
...
interface Node {
   ...
   */
  peerTxBroadcastConnection: bitcoreP2pSV.Peer;
  /**
   * Initially `undefined`, is set after the node connection is ready and
  ...
  async saveBlock(
    ...
    const { attemptedSavedTransactions, transactionCacheMisses } =
      await saveBlock({
        block,
        nodeAcceptances,
        transactionCache: this.transactionCache as never,
      });
